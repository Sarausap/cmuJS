    ...
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-12">
                ...
                <div class="card">
                    <div class="card-body">
                        <h4 class="card-title mb-3">Update Article</h4>
                        <form action="<?php echo base_url('admin/article/update/process'); ?>" method="post"
                            enctype="multipart/form-data">
                            <input type="hidden" name="article_id" value="<?php echo $article['articles_id'] ?>" />
                            <div class=" mb-3">
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" class="form-control" name="doi" required
                                        value="<?php echo $article["doi"] ?>">
                                </div>
                                <div class="col-md-6 mb-3 has-success">
                                    <label class="form-label">Volume</label>
                                    <select class="form-select" name="volume">
                                        <?php foreach ($volumes as $volume) {
                                            if ($article['volume_id'] == $volume['volume_id']) {
                                                echo "<option value='" . $volume["volume_id"] . "' selected='selected'>" . $volume["name"] . "</option>";
                                            } else {
                            ...
                            <div class="row">
                                                ...
                                                <div class="col">
                                                    <select class="choose form-select" name="author[]">
                                                        <?php foreach ($authors as $author): ?>
                                                            <option value="<?php echo $author['user_id']; ?>" <?php echo ($article_author['author_id'] == $author["user_id"]) ? 'selected' : ''; ?>>
                                                                <?php echo ($author['complete_name']); ?>
                                                            </option>
                                                        <?php endforeach; ?>
                                                    </select>
                                                </div>
                                ...
                                <div class="col-md-6">
                                    <input type="hidden" name="default_file"
                                        value="<?php echo $article["filename"] ?>" />
                                            ...
                                            <div class="keyword-input row">
                                                <div class="col">
                                                    <input type="text" class="form-control mt-3" id="keyword1"
                                                        name="keyword[]" placeholder="Type Keywords..." required <?php echo "value='" . $keyword . "'" ?>>
                                                </div>
                                                <div class="col-3 d-flex justify-content-center align-items-center">
                                                    <div class="mt-3 mt-md-0">
                                                        <button type="button" class="deleteKeyword btn hstack gap-6"
                                    ...
                                    <div class="col-lg-6 d-md-flex align-items-center justify-content-end">
                                        <div class="ms-auto mt-3 mt-md-0">
                                            <div class="form-check form-check-inline">
                                                <input class="form-check-input success" type="checkbox" name="status"
                                                    id="success2-check" <?php echo ($article['published'] == 1) ? "checked" : "" ?> />
                                                <label class="form-check-label" for="success2-check">Published</label>
                                            </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        editor.setData('<?php echo $article['abstract'] ?>');
    });

    document.getElementById('published').addEventListener('change', function () {
        var label = document.querySelector('.customLabel');
        if (this.checked) {
            this.classList.add('btn-success');
            label.textContent = 'Unpublish';
        } else {
            // Revert back when unchecked
